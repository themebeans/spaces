 ...
 */
function spaces_metaboxes_script( $hook ) {
	...
	wp_enqueue_script( 'spaces-metaboxes', get_theme_file_uri( '/assets/js/admin/metaboxes.js' ), array( 'jquery' ), '@@pkg.version', true );
	wp_enqueue_style( 'spaces-metaboxes', get_parent_theme_file_uri( '/assets/css/metaboxes.css' ), false, '@@pkg.version', 'all' );
}
add_action( 'admin_enqueue_scripts', 'spaces_metaboxes_script' );
	...
	foreach ( $meta_box['fields'] as $field ) {
		...
		switch ( $field['type'] ) {
							...
							insert: {
								...
								click: function() {
									var models = frame.state().get('selection'),
										...
										this.el.innerHTML = '<?php _e( 'Saving...', 'spaces' ); ?>';
			...
			?>
				...
				jQuery(function($) {
					...
					$('#bean_images_upload').on('click', function(e) {
						...
						//FIRST FRAME
						var options = {
							title: '<?php _e( 'Create Gallery', 'spaces' ); ?>',
							state: 'gallery-edit',
							frame: 'post',
							selection: selection
						};
						//IF FRAME IS THERE
						if( frame || selection ) {
							options['title'] = '<?php _e( 'Edit Gallery', 'spaces' ); ?>';
						}

						frame = wp.media(options).open();

						// CUSTOMIZE VIEWS
						frame.menu.get('view').get('cancel').el.innerHTML = '<?php _e( 'Cancel', 'spaces' ); ?>';
						frame.menu.get('view').get('gallery-edit').el.innerHTML = '<?php _e( 'Edit Gallery', 'spaces' ); ?>';
						...
						function overrideGalleryInsert() {
							frame.toolbar.get('view').set({
								insert: {
									style: 'primary',
									text: '<?php _e( 'Save Gallery', 'spaces' ); ?>',
			...
			<?php
				...
				$thumbnail_output = '';
				$button_text      = ( $meta ) ? esc_html__( 'Edit Gallery', 'spaces' ) : $field['std'];
			if ( $meta ) {
				$field['std']     = esc_html__( 'Edit Gallery', 'spaces' );
				$thumbs           = explode( ',', $meta );
				$thumbnail_output = '';
			...
			case 'checkbox':
                ...
                <input type="checkbox" id="' . $field['id'] . '" name="bean_meta[' . $field['id'] . ']" value="on"' . $val . ' /><span>' . esc_html__( 'Yes, please do', 'spaces' ) . '</span>';
				echo '</td>';
				break;
		}
